...
class SimplePerfLogger:
    def __init__(self):
        ...
        try:
            import config
            if hasattr(config, 'CONTAINER_LOG_MOUNT_PATH') and hasattr(config, 'CONTAINER_PERF_LOG_FILE'):
                log_dir = config.CONTAINER_LOG_MOUNT_PATH
                if os.path.exists(log_dir):
                    self.log_file = os.path.join(log_dir, 'container_perf.log')
                else:
                    self.log_file = config.CONTAINER_PERF_LOG_FILE
            else:
                # 回退到挂载的日志目录
                log_dir = '/tmp/faasflow_log'
                if os.path.exists(log_dir):
                    self.log_file = os.path.join(log_dir, 'container_perf.log')
                else:
                    self.log_file = os.environ.get('PERF_LOG_FILE', '/tmp/faasflow_container_perf.log')
        except ImportError:
            # 回退到挂载的日志目录
            log_dir = '/tmp/faasflow_log'
            if os.path.exists(log_dir):
